...
class StateCompressor:
    ...
    def add_reading(self, sim_time_str: str, outdoor_temp: float, zone_states: list, facility_kw: float):
        self.buffer.append({'time_str': sim_time_str, 'outdoor_temp': outdoor_temp, 'zones': zone_states, 'facility_kw': facility_kw})
    ...
    def compress(self, current_sim_minutes: float, error_summary=None):
        ...
        compressed_state = {'sim_time': latest_reading['time_str'], 'outdoor_temp_c': round(latest_reading['outdoor_temp'], 2), 'zones': latest_reading['zones'], 'facility_total_kw': round(latest_reading['facility_kw'], 2), 'rolling_30min_avg_kw': round(avg_facility_kw, 2), 'error_summary': error_summary or {'severe_count': 0, 'warning_count': 0, 'last_warnings': []}}
