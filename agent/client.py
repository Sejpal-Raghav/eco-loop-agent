...
# We assume Ollama is running locally
OLLAMA_URL = "http://localhost:11434/api/chat"
MODEL_NAME = "qwen2.5:7b-instruct" # Or llama3.1
    ...
    async def run_loop(self):
        ...
        async with stdio_client(self.server_params) as (read, write):
            async with ClientSession(read, write) as session:
                ...
                while True:
                    try:
                        # 1. Ask MCP server for current state
                        ...
                        # Only act if the simulation has advanced
                        if current_sim_time != last_sim_time:
                            ...
                            try:
                                ...
                                # 2. Send the proposed action back to the MCP server
                                ...
                                tool_result = await session.call_tool("propose_setpoint", {
                                    "zone": zone_name,
                                    "heating_c": heat,
                                    "cooling_c": cool
                                })
